...
Obrázok \ref{current-code-example} obsahuje ukážku súboru súčasnej aplikácie, na ktorom je možné vidieť skôr spomenuté miešanie logiky zodpovednej za funkcionalitu aplikácie (riadok 1--6), kód zodpovedný za prístup do databázy (riadok 10--19) a v neposlednom rade logiku zobrazovania aplikácie (riadok 21--41).
...
\subsection{Databázový model aplikácie}
...
Databáza súčasnej aplikácie sa skladá zo 6tich tabuliek: \mintinline{php}{czkp_mimi}, \mintinline{php}{czkp_vrh},\\ \mintinline{php}{czkp_zvire}, \mintinline{php}{pp_informace}, \mintinline{php}{pp_miminka} a \mintinline{php}{pp_zadosti}.
...
V nasledujúcich dvoch tabuľkách (\mintinline{php}{czkp_vrh} a \mintinline{php}{czkp_zvire}) sa vyskytujú tri rovnaké stĺpce, menovite \mintinline{php}{id_prvni_verze},  \mintinline{php}{datum_zaznamu} a  \mintinline{php}{uzivatel}. V stĺpci \mintinline{php}{id_prvni_verze} je uložené  \mintinline{php}{id} prvej verzie zvieraťa/vrhu. Následne v stĺpci \mintinline{php}{datum_zaznamu} je uložený dátum vytvorenia záznamu spolu s jeho časom a v stĺpci \mintinline{php}{uzivatel} identifikátor používateľa, ktorý daný záznam vytvoril. Tento identifikátor ukazuje na systémovú tabuľku \mintinline{php}{wp_users}, ktorá obsahuje informácie o používateľoch súčasného systému.
...
Táto tabuľka obsahuje stĺpec \mintinline{php}{uzivatel}, ktorý obsahuje identifikátor majiteľa vrhum ktorý ukazuje do tabuľky \mintinline{php}{wp_users}. Následne sa do tabuľky ukladajú údaje o samotnom vrhu, ako napríklad typ vrhu (\mintinline{php}{typ_vrhu}), jeho označenie (\mintinline{php}{vrh_oznaceni}), dátum narodenia (\mintinline{php}{vrh_narozeni}), varietnosť (\mintinline{php}{vrh_varietnost}), počet narodených a odchovaných mláďať (\mintinline{php}{vrh_nar_mladat} a \mintinline{php}{vrh_odch_mladat}) a počet odchovaných samcov a samíc (\mintinline{php}{vrh_odch_kluci} a \mintinline{php}{vrh_odch_holky}).
...
Informácie o chovatelovi vrhu sa ukladajú do dvoch stĺpcov, a to \mintinline{php}{chovatel} a \mintinline{php}{chov_kontakt}. Prvý z menovaných stĺpcov obsahuje meno chovateľa a druhý jeho kontakt.
